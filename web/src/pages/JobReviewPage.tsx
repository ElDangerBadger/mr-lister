        ...
        <fieldset disabled={saveState === "saving"}>
          ...
          <div className="tag-grid">
            {Array.from({ length: 13 }, (_, index) => {
              ...
              return (
                <div key={path}>
                  <label className="visually-hidden" htmlFor={`listing-tag-${index + 1}`}>Tag {index + 1}</label>
...
function ActionPanel({ review, reload, listingEditBarrier, approvalEvidenceAvailable, editActionsRef }: {
  ...
  const renderAction = (action: Exclude<SellerAction, "edit_listing">) => {
    const capability = capabilityFor(review, action);
...
function MockupGallery({ review, onAvailable, onUnavailable }: {
  ...
  return (
    <section className="panel mockups-panel" aria-labelledby="mockups-heading">
    ...
    <section className="panel economics-panel" aria-labelledby="economics-heading">
      ...
      {!displayable || economics.minimum_cents === null || economics.maximum_cents === null ? <p>Estimated proceeds are {humanLabel(economics.readiness).toLocaleLowerCase()}; no monetary estimate is presented.</p> : (
        <>
          ...
          <details><summary>Cost sources &amp; assumptions</summary>
