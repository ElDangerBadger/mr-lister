...
const contractVersion = z.literal("7.1.0");
...
export const publicationRequestResponseSchema = z.strictObject({
  ...
  if (Date.parse(value.verification_deadline) - Date.parse(value.requested_at) !== 30 * 60 * 1_000) {
...
export const sellerPublicationProjectionSchema = z.strictObject({
  contract_version: contractVersion,
  job_id: publicId,
  publication_enabled: z.literal(true),
  request_enabled: z.boolean(),
  request_disabled_reason: z.enum([
  ...
  if (value.request_enabled && (requested || value.request_disabled_reason !== null)) {
    context.addIssue({ code: "custom", path: ["request_enabled"], message: "Publication request capability is incoherent" });
  }
  if (!value.request_enabled && value.request_disabled_reason === null) {
    context.addIssue({ code: "custom", path: ["request_disabled_reason"], message: "Disabled publication request has no reason" });
  }
  if (requested && value.request_disabled_reason !== "PUBLICATION_ALREADY_REQUESTED") {
    context.addIssue({ code: "custom", path: ["request_disabled_reason"], message: "Requested publication has the wrong disabled reason" });
  }
  if (!requested && !value.request_enabled && value.request_disabled_reason !== "PUBLICATION_NOT_ELIGIBLE") {
    context.addIssue({ code: "custom", path: ["request_disabled_reason"], message: "Ineligible publication has the wrong disabled reason" });
  }
