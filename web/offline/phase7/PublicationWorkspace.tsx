...
import type { SellerReview } from "../../src/contracts";
import {
  PublicationApiError,
...
interface PublicationWorkspaceProps {
  ...
  /** Test-only authority for the unregistered oracle. Frozen runtime composition never sets it. */
  requestAvailableForOfflineOracle?: boolean;
}
...
export function PublicationWorkspace({
  ...
  approvedReview,
  api,
  requestAvailableForOfflineOracle = false,
  ...
  return (
    <section className="panel" aria-labelledby="phase7-publication-heading" data-phase7-publication-workspace="offline">
      ...
      {projection?.state === "not_requested" && !mayRequest && !requestBlocked && (
        <p>Seller publication is not activated for this application.</p>
      )}
      {confirming && (
        <dialog
          ref={dialog}
