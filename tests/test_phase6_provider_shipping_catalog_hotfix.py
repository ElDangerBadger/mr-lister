...
import tools.render_phase6_provider_shipping_catalog_hotfix as hotfix

VERSION = "provider-shipping-version_A1"
...
def _binding(version: str = VERSION) -> hotfix.Phase6ProviderShippingCatalogHotfixBinding:
    return hotfix.Phase6ProviderShippingCatalogHotfixBinding(
...
def _render(
    ...
    rendered = hotfix.render_phase6_provider_shipping_catalog_hotfix(raw, _binding())
...
def test_render_changes_only_provider_key_version_release_and_provenance(
    ...
    assert provider_before["Properties"]["CodeUri"] == hotfix._code_uri(
        hotfix.PROVIDER_CATALOG_EXPANSION_RELEASE_FINGERPRINT,
        hotfix.PROVIDER_CATALOG_EXPANSION_LAMBDA_ARCHIVE_SHA256,
        hotfix.PROVIDER_CATALOG_EXPANSION_LAMBDA_VERSION_ID,
    )
    assert provider_before["Properties"]["Environment"]["Variables"] == {
        hotfix._RELEASE_ENVIRONMENT_KEY: (hotfix.PROVIDER_CATALOG_EXPANSION_RELEASE_FINGERPRINT)
    }
...
def test_release_source_archive_and_predecessor_are_fixed() -> None:
    assert hotfix.SOURCE_COMMIT == "15a4f2a657e4cf5809de7066d267455d65c8c835"
    ...
    assert hotfix.TARGET_LAMBDA_ARCHIVE_SIZE == 62_710_255
    assert hotfix.TARGET_LAMBDA_ARTIFACT_KEY == (
        "private/deployments/lambda/releases/"
        "748e5c4a1e46c500215118685d1f70231b7f28b8bfe8e67cc804da1c33e7c347/"
        "phase6-lambda-0a12ede1b86c47069fda57938d1b0d50aeab6acca5d04ab536446d2f414de405.zip"
    )
...
def test_binding_rejects_nonliteral_or_moving_version(version: str) -> None:
    with pytest.raises(hotfix.Phase6ProviderShippingCatalogHotfixError):
        _binding(version)
...
def test_render_rejects_byte_drift_duplicate_json_and_wrong_binding_type(
    ...
    with pytest.raises(hotfix.Phase6ProviderShippingCatalogHotfixError):
        hotfix.render_phase6_provider_shipping_catalog_hotfix(
    ...
    with pytest.raises(hotfix.Phase6ProviderShippingCatalogHotfixError):
        hotfix.render_phase6_provider_shipping_catalog_hotfix(duplicate, _binding())
...
def _bind_private_write(
    ...
    rendered = hotfix.render_phase6_provider_shipping_catalog_hotfix(
        predecessor_raw,
        _binding(),
    )
...
def test_private_write_is_create_only_owner_only_and_separately_verifiable(
    ...
    assert (
        hotfix.write_phase6_provider_shipping_catalog_hotfix(
            _binding(),
            output_path=output,
        )
        == output
    ...
    assert (
        hotfix.verify_phase6_provider_shipping_catalog_hotfix(
            _binding(),
            output_path=output,
        )
        == output
...
def test_cli_writes_and_verifies_the_exact_version(
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "render_phase6_provider_shipping_catalog_hotfix",
