...
import tools.render_phase6_provider_catalog_expansion_hotfix as hotfix

VERSION = "provider-catalog-version_A1"
...
def _binding(version: str = VERSION) -> hotfix.Phase6ProviderCatalogExpansionHotfixBinding:
    return hotfix.Phase6ProviderCatalogExpansionHotfixBinding(
...
def _render(
    ...
    rendered = hotfix.render_phase6_provider_catalog_expansion_hotfix(raw, _binding())
...
def test_release_source_archive_and_predecessor_are_fixed() -> None:
    assert hotfix.SOURCE_COMMIT == "81623940b5d867668d4aa92aa4953db7706908bb"
    ...
    assert hotfix.TARGET_LAMBDA_ARCHIVE_SIZE == 62_708_849
    assert hotfix.TARGET_LAMBDA_ARTIFACT_KEY == (
        "private/deployments/lambda/releases/"
        "3bfccee40d144f284827da221df40a787f7d09242698d177b51bbcc1414b7308/"
        "phase6-lambda-e5f17ee5063a2a6aaf490df899d7c98d5a79c97b6d29911260214a6ac599cc9b.zip"
    )
...
def test_binding_rejects_nonliteral_or_moving_version(version: str) -> None:
    with pytest.raises(hotfix.Phase6ProviderCatalogExpansionHotfixError):
        _binding(version)
...
def test_render_rejects_byte_drift_duplicate_json_and_wrong_binding_type(
    ...
    with pytest.raises(hotfix.Phase6ProviderCatalogExpansionHotfixError):
        hotfix.render_phase6_provider_catalog_expansion_hotfix(
    ...
    with pytest.raises(hotfix.Phase6ProviderCatalogExpansionHotfixError):
        hotfix.render_phase6_provider_catalog_expansion_hotfix(duplicate, _binding())
...
def _bind_private_write(
    ...
    rendered = hotfix.render_phase6_provider_catalog_expansion_hotfix(
        predecessor_raw,
        _binding(),
    )
...
def test_private_write_is_create_only_owner_only_and_separately_verifiable(
    ...
    assert (
        hotfix.write_phase6_provider_catalog_expansion_hotfix(
            _binding(),
            output_path=output,
        )
        == output
    ...
    assert (
        hotfix.verify_phase6_provider_catalog_expansion_hotfix(
            _binding(),
            output_path=output,
        )
        == output
...
def test_cli_writes_and_verifies_the_exact_version(
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "render_phase6_provider_catalog_expansion_hotfix",
