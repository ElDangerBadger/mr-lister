# Phase 6.4 AWS infrastructure scaffold
...
`AgentCoreRuntimeArn`, `PrintifySecretArn`, `ApplicationOrigin`, `ApplicationCallbackUrl`, and
`ApplicationLogoutUrl` are required, nonblank, exact-resource or exact-origin parameters. The
browser upload CORS rule and API CORS policy accept only `ApplicationOrigin`; neither uses a
wildcard. The HTTP API has exactly thirteen authenticated `/v1` routes and one public,
information-minimal `/health` route.
