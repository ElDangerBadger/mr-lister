...
`branding.css` and `logo.jpg` are a cosmetic candidate for the existing Cognito
Hosted UI (classic) app client. They are not part of the React bundle and have not
been applied to Cognito. The original PNG in `web/src/assets` remains unchanged;
the JPEG is a 360 × 360 encoding of that artwork for Cognito's upload size limit.

The candidate uses the application's green buttons, dark green labels and error
...
replacement of the hosted login layout. Login and MFA must be observed live
after application before claiming full visual acceptance.
...
image base64 encoding. This candidate is 45,001 image bytes and 1,928 CSS bytes.
...
production source. The local preview was not visually verified because browser
URL policy rejected opening its file URL; do not treat it as acceptance evidence.
