"""Render the exact provider-only Phase 6 shipping-catalog hotfix.

This offline renderer starts from the byte-sealed provider-catalog application template and
...
DEFAULT_PREDECESSOR_PATH: Final = REPOSITORY_ROOT / (
    ".mr_lister_private/phase6-provider-normalization-8162394-20260901T165309Z/"
    "template.provider-catalog-expansion-hotfix.local.json"
)
DEFAULT_OUTPUT_PATH: Final = REPOSITORY_ROOT / (
    ".mr_lister_private/phase6-provider-shipping-catalog-hotfix/"
    "template.provider-shipping-catalog-hotfix.local.json"
)

SOURCE_COMMIT: Final = "15a4f2a657e4cf5809de7066d267455d65c8c835"
...
TARGET_LAMBDA_ARCHIVE_SIZE: Final = 62_710_255
...
PROVIDER_CATALOG_EXPANSION_RELEASE_FINGERPRINT: Final = (
    "3bfccee40d144f284827da221df40a787f7d09242698d177b51bbcc1414b7308"
)
PROVIDER_CATALOG_EXPANSION_LAMBDA_ARCHIVE_SHA256: Final = (
    "e5f17ee5063a2a6aaf490df899d7c98d5a79c97b6d29911260214a6ac599cc9b"
)
PROVIDER_CATALOG_EXPANSION_LAMBDA_VERSION_ID: Final = "yKXCfg0bAMU0Kyu6xAoM2hXtS3KxxHkC"
...
PROVIDER_SHIPPING_CATALOG_HOTFIX_FORMAT: Final = (
    "mr-lister-phase6-provider-shipping-catalog-hotfix-v1"
)
PREDECESSOR_RESOURCE_COUNT: Final = 102
DEPLOYED_PROCESSED_RESOURCE_COUNT: Final = 125
_METADATA_KEY: Final = "MrListerPhase6ProviderShippingCatalogHotfix"
...
_GENERIC_ERROR: Final = "Phase 6 provider shipping-catalog hotfix is invalid"


class Phase6ProviderShippingCatalogHotfixError(RuntimeError):
    """A value-free rendering or confinement failure."""


@dataclass(frozen=True, slots=True)
class Phase6ProviderShippingCatalogHotfixBinding:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise Phase6ProviderShippingCatalogHotfixError(_GENERIC_ERROR) from None
...
def _expected_predecessor_function_binding(logical_id: str) -> tuple[dict[str, str], str | None]:
    if logical_id == "ProviderDraftFunction":
        return (
            _code_uri(
                PROVIDER_CATALOG_EXPANSION_RELEASE_FINGERPRINT,
                PROVIDER_CATALOG_EXPANSION_LAMBDA_ARCHIVE_SHA256,
                PROVIDER_CATALOG_EXPANSION_LAMBDA_VERSION_ID,
            ),
            PROVIDER_CATALOG_EXPANSION_RELEASE_FINGERPRINT,
        )
...
def _provenance(binding: Phase6ProviderShippingCatalogHotfixBinding) -> dict[str, object]:
    return {
        "Format": PROVIDER_SHIPPING_CATALOG_HOTFIX_FORMAT,
        "LambdaArtifact": {
            "Bucket": LAMBDA_ARTIFACT_BUCKET,
            "Key": TARGET_LAMBDA_ARTIFACT_KEY,
...
def render_phase6_provider_shipping_catalog_hotfix(
    predecessor_raw: bytes,
    binding: Phase6ProviderShippingCatalogHotfixBinding,
) -> bytes:
    """Return the canonical provider-only application target."""

    try:
        if not isinstance(binding, Phase6ProviderShippingCatalogHotfixBinding):
            raise ValueError
    ...
    except Phase6ProviderShippingCatalogHotfixError:
        raise
    except Exception:
        raise Phase6ProviderShippingCatalogHotfixError(_GENERIC_ERROR) from None
...
def write_phase6_provider_shipping_catalog_hotfix(
    binding: Phase6ProviderShippingCatalogHotfixBinding,
    *,
    ...
    try:
        rendered = render_phase6_provider_shipping_catalog_hotfix(
...
def verify_phase6_provider_shipping_catalog_hotfix(
    binding: Phase6ProviderShippingCatalogHotfixBinding,
    *,
    output_path: Path = DEFAULT_OUTPUT_PATH,
) -> Path:
    ...
    try:
        expected = render_phase6_provider_shipping_catalog_hotfix(
            _read_repository_file(DEFAULT_PREDECESSOR_PATH),
            binding,
        )
...
def main() -> int:
    ...
    try:
        ...
        output = (
            write_phase6_provider_shipping_catalog_hotfix(
                binding,
                output_path=arguments.output,
            )
            if arguments.write
            else verify_phase6_provider_shipping_catalog_hotfix(
                binding,
                output_path=arguments.output,
            )
        )
        rendered = _read_repository_file(output)
    except Phase6ProviderShippingCatalogHotfixError as error:
        print(str(error))
        return 2
    print(
        json.dumps(
