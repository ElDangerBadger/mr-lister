...
_CHANGE_SET_ID = re.compile(
    ...
    r"mr-lister-phase6-dev-dispatch-filter-[a-f0-9]{12}/[0-9a-f-]{36}$"
)
...
_TARGET_ALLOWED_KEYS: Final = _TARGET_REQUIRED_KEYS | {"AfterValue", "BeforeValue"}
...
def verify_phase6_dispatch_filter_change_set(
    *, change_set: Mapping[str, Any]
    ...
    """Reject anything other than the reviewed in-place FilterCriteria modification."""
    ...
    try:
        ...
        if (
            ...
            or not _TARGET_REQUIRED_KEYS <= set(target) <= _TARGET_ALLOWED_KEYS
            ...
            raise ValueError
        expected_after = {"Filters": [{"Pattern": json.dumps(SAFE_FILTER, separators=(",", ":"))}]}
