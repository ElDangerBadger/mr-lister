"""Render the exact bounded Phase 6 post-walkthrough application hotfix.

This offline renderer starts from the byte-sealed deployed artwork-closure template.  It advances
only the two Lambda functions exercised after upload, their function-level release fingerprints,
the four immutable AgentCore endpoint parameters consumed by preparation, and one provenance
record.  Upload, review-query, every other Lambda, all web-edge resources, and the global release
parameter remain on their deployed mosaic.
...
TARGET_LAMBDA_ARCHIVE_SIZE: Final = 62_703_275

AGENTCORE_RUNTIME_ARN: Final = (
    "arn:aws:bedrock-agentcore:us-west-2:384627057108:runtime/mr_lister_phase6-4HoPmq2hCI"
)
PREDECESSOR_AGENTCORE_RUNTIME_VERSION: Final = "3"
PREDECESSOR_AGENTCORE_QUALIFIER: Final = "phase6_v3_dev"
PREDECESSOR_AGENTCORE_ENDPOINT_ARN: Final = (
    f"{AGENTCORE_RUNTIME_ARN}/runtime-endpoint/{PREDECESSOR_AGENTCORE_QUALIFIER}"
)
PREDECESSOR_AGENTCORE_BINDING_FINGERPRINT: Final = (
    "d8194386435d2f941d0942b102595830c1efc48e9bc4890457b46e17e0df3196"
)
TARGET_AGENTCORE_RUNTIME_VERSION: Final = "4"
TARGET_AGENTCORE_QUALIFIER: Final = "phase6_v4_dev"
...
APPLICATION_WALKTHROUGH_HOTFIX_FORMAT: Final = "mr-lister-phase6-application-walkthrough-hotfix-v1"
# The sealed SAM source contains 102 resources.  CloudFormation's processed form expands the same
# topology to the 125 resources observed in the deployed stack; this renderer never adds/removes a
# source resource, so the processed topology remains unchanged as well.
PREDECESSOR_RESOURCE_COUNT: Final = 102
DEPLOYED_PROCESSED_RESOURCE_COUNT: Final = 125
_METADATA_KEY: Final = "MrListerPhase6ApplicationWalkthroughHotfix"
...
_GENERIC_ERROR: Final = "Phase 6 application walkthrough hotfix is invalid"


class Phase6ApplicationWalkthroughHotfixError(RuntimeError):
    """A value-free rendering or confinement failure."""


@dataclass(frozen=True, slots=True)
class Phase6ApplicationWalkthroughHotfixBinding:
    """Exact immutable Lambda object and AgentCore v4 endpoint authority."""
    ...
    def __post_init__(self) -> None:
        try:
            ...
            if (
                ...
                or key_match.groups() != (TARGET_RELEASE_FINGERPRINT, TARGET_LAMBDA_ARCHIVE_SHA256)
            ...
            raise Phase6ApplicationWalkthroughHotfixError(_GENERIC_ERROR) from None
...
def _expected_predecessor_function_binding(logical_id: str) -> tuple[dict[str, str], str | None]:
    if logical_id in _ARTWORK_CLOSURE_FUNCTION_IDS:
        return (
            _code_uri(
    ...
    return (
        _code_uri(
            BASE_RELEASE_FINGERPRINT,
            BASE_LAMBDA_ARCHIVE_SHA256,
            BASE_LAMBDA_VERSION_ID,
        ),
        None,
    )
...
def _validate_predecessor(predecessor: dict[str, Any]) -> None:
    ...
    _locked_parameter(parameters, "ReleaseFingerprint", BASE_RELEASE_FINGERPRINT)
    ...
    _locked_parameter(
        parameters,
        "AgentCoreRuntimeQualifier",
        PREDECESSOR_AGENTCORE_QUALIFIER,
    )
    _locked_parameter(
        parameters,
        "AgentCoreRuntimeVersion",
        PREDECESSOR_AGENTCORE_RUNTIME_VERSION,
    )
    ...
    if _METADATA_KEY in metadata or readiness.get("Value") != "WEB_EDGE_ACTIVE_DRAFT_ONLY":
        raise ValueError
...
def _provenance(binding: Phase6ApplicationWalkthroughHotfixBinding) -> dict[str, object]:
    return {
        "AgentCore": {
        ...
        "Format": APPLICATION_WALKTHROUGH_HOTFIX_FORMAT,
        "LambdaArtifact": {
        ...
        "TargetFunctions": list(_TARGET_FUNCTION_IDS),
    }


def render_phase6_application_walkthrough_hotfix(
    predecessor_raw: bytes,
    binding: Phase6ApplicationWalkthroughHotfixBinding,
) -> bytes:
    """Return the canonical two-Lambda plus AgentCore v4 application target."""

    try:
        if not isinstance(binding, Phase6ApplicationWalkthroughHotfixBinding):
            raise ValueError
        ...
        for logical_id in _TARGET_FUNCTION_IDS:
            resource = _mapping(resources.get(logical_id))
            properties = _mapping(resource.get("Properties"))
        ...
        expected_paths: set[tuple[str, ...]] = {("Metadata", _METADATA_KEY)}
        for logical_id in _TARGET_FUNCTION_IDS:
    ...
    except Phase6ApplicationWalkthroughHotfixError:
        raise
    except Exception:
        raise Phase6ApplicationWalkthroughHotfixError(_GENERIC_ERROR) from None
...
def write_phase6_application_walkthrough_hotfix(
    binding: Phase6ApplicationWalkthroughHotfixBinding,
    *,
    ...
    try:
        rendered = render_phase6_application_walkthrough_hotfix(
...
def verify_phase6_application_walkthrough_hotfix(
    binding: Phase6ApplicationWalkthroughHotfixBinding,
    *,
    output_path: Path = DEFAULT_OUTPUT_PATH,
) -> Path:
    ...
    try:
        expected = render_phase6_application_walkthrough_hotfix(
            _read_repository_file(DEFAULT_PREDECESSOR_PATH),
            binding,
        )
...
def main() -> int:
    ...
    try:
        ...
        output = (
            write_phase6_application_walkthrough_hotfix(binding, output_path=arguments.output)
            if arguments.write
            else verify_phase6_application_walkthrough_hotfix(
                binding,
                output_path=arguments.output,
            )
        )
        rendered = _read_repository_file(output)
    except Phase6ApplicationWalkthroughHotfixError as error:
        print(str(error))
        return 2
    print(
        json.dumps(
