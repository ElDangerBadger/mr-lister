"""Render the exact Phase 6 AgentCore closure update without calling AWS.

The renderer is intentionally specific to the accepted development runtime and the sealed
Phase 6 closure artifact.  It has no account, Region, runtime, version, role, model, product, or
output overrides.  Runtime-update inputs are emitted only after fresh v2 and retained-role
observations plus the closed common-v2 S3 object evidence verify.  Endpoint inputs are a second
stage and additionally require joined update/get evidence showing runtime v3 READY.
"""
...
TARGET_VERSION: Final = "3"
CURRENT_ENDPOINT_NAME: Final = "phase6_v2_dev"
TARGET_ENDPOINT_NAME: Final = "phase6_v3_dev"
...
PREDECESSOR_RELEASE_FINGERPRINT: Final = (
    "2c1b2b5e47994832f0cd1be9fb8088e2b0b7e7be7aa18a117c09f5530fb7c549"
)
PREDECESSOR_ARCHIVE_SHA256: Final = (
    "90a081eaccbfacf0ecb170c7a295340fe86e5b7c8951e4474400db46f7b26ef5"
)
PREDECESSOR_S3_VERSION_ID: Final = "B64_bDuTGgc2a4K1PrLNWdSBqOeJpOo6"

CLOSURE_RELEASE_FINGERPRINT: Final = (
    "f34ab73042014fccce2cb3733624f005a4ccc10bb065b39c3e20befd3c33923f"
)
CLOSURE_ARCHIVE_SHA256: Final = "443f62fe01a2ebd54c8ff4b551eab94c829a878b42333973a6731e1cdd105f8b"
CLOSURE_ARCHIVE_SIZE: Final = 96_310_832
...
CLOSURE_KEY: Final = (
    "private/deployments/agentcore/releases/"
    f"{CLOSURE_RELEASE_FINGERPRINT}/phase6-agentcore-{CLOSURE_ARCHIVE_SHA256}.zip"
)
...
PREDECESSOR_EVIDENCE_FILE: Final = EVIDENCE_DIRECTORY / "agentcore-runtime-v2-observation.json"
ROLE_EVIDENCE_FILE: Final = EVIDENCE_DIRECTORY / "agentcore-runtime-role-v3-observation.json"
TARGET_EVIDENCE_FILE: Final = EVIDENCE_DIRECTORY / "agentcore-runtime-v3-update-evidence.json"
OUTPUT_DIRECTORY: Final = Path("agentcore-v3-deployment")
RUNTIME_UPDATE_FILE: Final = OUTPUT_DIRECTORY / "update-agent-runtime-v3.local.json"
RUNTIME_MANIFEST_FILE: Final = OUTPUT_DIRECTORY / "runtime-update-manifest.local.json"
ENDPOINT_CREATE_FILE: Final = OUTPUT_DIRECTORY / "create-agent-runtime-endpoint-v3.local.json"
ENDPOINT_MANIFEST_FILE: Final = OUTPUT_DIRECTORY / "endpoint-create-manifest.local.json"

PREDECESSOR_EVIDENCE_FORMAT: Final = "mr-lister-phase6-agentcore-v2-observation-v1"
ROLE_EVIDENCE_FORMAT: Final = "mr-lister-phase6-agentcore-runtime-role-v3-observation-v1"
TARGET_EVIDENCE_FORMAT: Final = "mr-lister-phase6-agentcore-v3-update-evidence-v1"
RUNTIME_MANIFEST_FORMAT: Final = "mr-lister-phase6-agentcore-v3-update-render-v1"
ENDPOINT_MANIFEST_FORMAT: Final = "mr-lister-phase6-agentcore-v3-endpoint-render-v1"

MAX_OBSERVATION_AGE: Final = timedelta(minutes=15)
MAX_FUTURE_SKEW: Final = timedelta(minutes=2)
_CLOSURE_ROOT = re.compile(r"^phase6-closure-[a-f0-9]{7,40}-[0-9]{8}T[0-9]{6}Z$")
...
_GENERIC_ERROR: Final = "Phase 6 AgentCore closure update input is invalid"


class Phase6AgentCoreClosureUpdateError(RuntimeError):
    """A value-free failure for incomplete, drifting, stale, or unsafe closure input."""
...
class VerifiedObservation:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise Phase6AgentCoreClosureUpdateError(_GENERIC_ERROR)
...
def verify_phase6_agentcore_closure_context(
    closure_root: Path,
    *,
    repository_root: Path = ROOT,
) -> ClosureContext:
    """Verify the fixed closure root, sealed archive, embedded config, and remote object proof."""

    try:
        repository, closure = _resolve_closure_root(repository_root, closure_root)
        ...
        if archive.sha256 != CLOSURE_ARCHIVE_SHA256 or archive.size_bytes != CLOSURE_ARCHIVE_SIZE:
            raise ValueError
        ...
        remote = verify_phase6_s3_release_object_evidence(
            Phase6S3ReleaseObjectExpectation(
                ...
                component="agentcore",
                release_fingerprint=CLOSURE_RELEASE_FINGERPRINT,
                archive_sha256=CLOSURE_ARCHIVE_SHA256,
                size_bytes=CLOSURE_ARCHIVE_SIZE,
            ),
        ...
        if (
            ...
            or remote.key != CLOSURE_KEY
            or remote.release_fingerprint != CLOSURE_RELEASE_FINGERPRINT
            or remote.archive_sha256 != CLOSURE_ARCHIVE_SHA256
            or remote.size_bytes != CLOSURE_ARCHIVE_SIZE
        ):
            raise ValueError
        return ClosureContext(closure_root=closure, archive=archive, remote=remote)
    except Phase6AgentCoreClosureUpdateError:
        raise
    except Exception:
        raise Phase6AgentCoreClosureUpdateError(_GENERIC_ERROR) from None


def verify_phase6_agentcore_v2_observation(
    path: Path,
    *,
    now: datetime | None = None,
) -> VerifiedObservation:
    """Verify a recent exact READY v2 runtime and immutable v2 endpoint readback."""
...
def render_phase6_agentcore_runtime_update_documents(
    ...
    *,
    ...
    """Render exact v3 update bytes after every local, S3, v2, and role prerequisite verifies."""

    try:
        context = verify_phase6_agentcore_closure_context(
            closure_root, repository_root=repository_root
        )
        predecessor_path = context.closure_root / PREDECESSOR_EVIDENCE_FILE
        role_path = context.closure_root / ROLE_EVIDENCE_FILE
        _require_private_file(repository_root, context.closure_root, predecessor_path)
        _require_private_file(repository_root, context.closure_root, role_path)
        predecessor = verify_phase6_agentcore_v2_observation(predecessor_path, now=now)
        ...
        manifest = {
            ...
            "preconditions": {
                ...
                "readyRuntimeV2EvidenceSHA256": predecessor.evidence_sha256,
...
def verify_phase6_agentcore_v3_update_evidence(
    path: Path,
    *,
    ...
    """Verify update response and READY v3 readback against the exact rendered update bytes."""
...
def render_phase6_agentcore_endpoint_documents(
    ...
    *,
    ...
    """Render immutable v3 endpoint bytes only after the exact update is observed READY."""
    ...
    try:
        ...
        predecessor_path = context.closure_root / PREDECESSOR_EVIDENCE_FILE
        role_path = context.closure_root / ROLE_EVIDENCE_FILE
        target_path = context.closure_root / TARGET_EVIDENCE_FILE
        for path in (predecessor_path, role_path, target_path):
            _require_private_file(repository_root, context.closure_root, path)
        predecessor = verify_phase6_agentcore_v2_observation(predecessor_path, now=now)
        role = verify_phase6_agentcore_runtime_role_observation(
            role_path,
            now=now,
        ...
        target = verify_phase6_agentcore_v3_update_evidence(
            target_path,
...
def write_phase6_agentcore_runtime_update_documents(
    ...
    *,
    ...
    documents = render_phase6_agentcore_runtime_update_documents(
        closure_root, repository_root=repository_root, now=now
    )
    _, closure = _resolve_closure_root(repository_root, closure_root)
...
def _binding() -> Phase6AgentCoreDirectCodeZipBinding:
    return Phase6AgentCoreDirectCodeZipBinding(
        account_id=ACCOUNT_ID,
        release_fingerprint=CLOSURE_RELEASE_FINGERPRINT,
        agentcore_archive_sha256=CLOSURE_ARCHIVE_SHA256,
    )
...
def _runtime_update_input(
    ...
    return {
        ...
        "environmentVariables": _environment(CLOSURE_RELEASE_FINGERPRINT),
...
def _binding_document() -> dict[str, object]:
    return {
        ...
        "protocolConfiguration": PROTOCOL_CONFIGURATION,
        "region": REGION,
        "releaseFingerprint": CLOSURE_RELEASE_FINGERPRINT,
        "roleArn": ROLE_ARN,
...
def _expected_role_documents(
    ...
    replacements = {
        ...
        "AWS::Partition": "aws",
        "AgentCoreArchiveSha256": CLOSURE_ARCHIVE_SHA256,
        "ReleaseFingerprint": CLOSURE_RELEASE_FINGERPRINT,
    }
...
def _validate_exact_role_s3_boundary(policy: Mapping[str, object]) -> None:
    ...
    if set(by_sid) != {
        ...
        raise ValueError
    if by_sid["ReadOnlyExactAgentCoreDeploymentArchive"] != {
        "Action": ["s3:GetObject", "s3:GetObjectVersion"],
        "Effect": "Allow",
        "Resource": f"arn:aws:s3:::{ARTIFACT_BUCKET}/{CLOSURE_KEY}",
...
def _validate_runtime_response(
    ...
    *,
    ...
    expected_key = PREDECESSOR_KEY if version == CURRENT_VERSION else CLOSURE_KEY
    if (
...
def _client_token(
    ...
    *evidence_digests: str,
    ...
    material = _canonical_json(
        {
            "accountId": ACCOUNT_ID,
            "archiveSHA256": CLOSURE_ARCHIVE_SHA256,
            "currentVersion": CURRENT_VERSION,
            "evidenceSHA256": list(evidence_digests),
            "format": f"mr-lister-phase6-agentcore-v3-{kind}-token-v1",
            "releaseFingerprint": CLOSURE_RELEASE_FINGERPRINT,
            "runtimeId": RUNTIME_ID,
    ...
    return f"mr-lister-phase6-{kind}-v3-{sha256(material).hexdigest()[:32]}"


def _resolve_closure_root(repository_root: Path, closure_root: Path) -> tuple[Path, Path]:
    if not isinstance(repository_root, Path) or not isinstance(closure_root, Path):
        raise Phase6AgentCoreClosureUpdateError(_GENERIC_ERROR)
    repository = repository_root.resolve(strict=True)
    ...
    if private.is_symlink() or not private.is_dir():
        raise Phase6AgentCoreClosureUpdateError(_GENERIC_ERROR)
    if closure_root.is_symlink():
        raise Phase6AgentCoreClosureUpdateError(_GENERIC_ERROR)
    closure = closure_root.resolve(strict=True)
    if (
        closure.parent != private.resolve(strict=True)
        or _CLOSURE_ROOT.fullmatch(closure.name) is None
...
def _require_private_file(repository_root: Path, closure_root: Path, path: Path) -> None:
    repository, closure = _resolve_closure_root(repository_root, closure_root)
...
def _write_new_documents(
    ...
    *,
    ...
    try:
        output = closure_root / OUTPUT_DIRECTORY
        if require_output_absent and (output.exists() or output.is_symlink()):
            raise ValueError
        if not require_output_absent:
            if output.is_symlink() or not output.is_dir():
                raise ValueError
            existing = {path.relative_to(closure_root) for path in output.iterdir()}
            if existing != {RUNTIME_MANIFEST_FILE, RUNTIME_UPDATE_FILE}:
                raise ValueError
        destinations = tuple(closure_root / relative for relative in documents)
...
def _verify_written_documents(
    ...
    *,
    ...
    try:
        ...
        entries = {path.relative_to(closure_root) for path in output.iterdir()}
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Render or verify the exact private Phase 6 AgentCore v2-to-v3 closure inputs"
    )
    ...
    try:
        if arguments.write_runtime_update:
            write_phase6_agentcore_runtime_update_documents(arguments.closure_root)
        elif arguments.verify_runtime_update:
            verify_phase6_agentcore_runtime_update_documents(arguments.closure_root)
        elif arguments.write_endpoint:
            write_phase6_agentcore_endpoint_documents(arguments.closure_root)
        else:
            verify_phase6_agentcore_endpoint_documents(arguments.closure_root)
        return 0
    except Phase6AgentCoreClosureUpdateError as error:
