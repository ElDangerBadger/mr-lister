"""Render the exact provider-only Phase 6 reconciliation hotfix.

This offline renderer starts from the byte-sealed provider-shipping application template and
...
DEFAULT_PREDECESSOR_PATH: Final = REPOSITORY_ROOT / (
    ".mr_lister_private/phase6-shipping-normalization-15a4f2a-20260901T174410Z/"
    "template.provider-shipping-catalog-hotfix.local.json"
)
DEFAULT_OUTPUT_PATH: Final = REPOSITORY_ROOT / (
    ".mr_lister_private/phase6-provider-reconciliation-hotfix/"
    "template.provider-reconciliation-hotfix.local.json"
)

SOURCE_COMMIT: Final = "bd9f3686ef812621f59a5bf031902d8ef5a88208"
...
TARGET_LAMBDA_ARCHIVE_SIZE: Final = 62_710_862
...
PROVIDER_SHIPPING_CATALOG_RELEASE_FINGERPRINT: Final = (
    "748e5c4a1e46c500215118685d1f70231b7f28b8bfe8e67cc804da1c33e7c347"
)
PROVIDER_SHIPPING_CATALOG_LAMBDA_ARCHIVE_SHA256: Final = (
    "0a12ede1b86c47069fda57938d1b0d50aeab6acca5d04ab536446d2f414de405"
)
PROVIDER_SHIPPING_CATALOG_LAMBDA_VERSION_ID: Final = "w60dzk3jW5BZVxy0_SfoKWYVy_3.oL8g"
...
PROVIDER_RECONCILIATION_HOTFIX_FORMAT: Final = "mr-lister-phase6-provider-reconciliation-hotfix-v1"
PREDECESSOR_RESOURCE_COUNT: Final = 102
DEPLOYED_PROCESSED_RESOURCE_COUNT: Final = 125
_METADATA_KEY: Final = "MrListerPhase6ProviderReconciliationHotfix"
_PREDECESSOR_METADATA_KEYS: Final = frozenset(
    {
        "MrListerPhase6ApplicationWalkthroughHotfix",
        "MrListerPhase6CoreRuntimeStaging",
        "MrListerPhase6ProviderCatalogExpansionHotfix",
        "MrListerPhase6ProviderShippingCatalogHotfix",
...
_GENERIC_ERROR: Final = "Phase 6 provider reconciliation hotfix is invalid"


class Phase6ProviderReconciliationHotfixError(RuntimeError):
    """A value-free rendering or confinement failure."""


@dataclass(frozen=True, slots=True)
class Phase6ProviderReconciliationHotfixBinding:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise Phase6ProviderReconciliationHotfixError(_GENERIC_ERROR) from None
...
def _expected_predecessor_function_binding(logical_id: str) -> tuple[dict[str, str], str | None]:
    if logical_id == "ProviderDraftFunction":
        return (
            _code_uri(
                PROVIDER_SHIPPING_CATALOG_RELEASE_FINGERPRINT,
                PROVIDER_SHIPPING_CATALOG_LAMBDA_ARCHIVE_SHA256,
                PROVIDER_SHIPPING_CATALOG_LAMBDA_VERSION_ID,
            ),
            PROVIDER_SHIPPING_CATALOG_RELEASE_FINGERPRINT,
        )
    if logical_id == "PreparationDispatchFunction":
        return (
            _code_uri(
...
def _provenance(binding: Phase6ProviderReconciliationHotfixBinding) -> dict[str, object]:
    return {
        "Format": PROVIDER_RECONCILIATION_HOTFIX_FORMAT,
        "LambdaArtifact": {
            "Bucket": LAMBDA_ARTIFACT_BUCKET,
            "Key": TARGET_LAMBDA_ARTIFACT_KEY,
...
def render_phase6_provider_reconciliation_hotfix(
    predecessor_raw: bytes,
    binding: Phase6ProviderReconciliationHotfixBinding,
) -> bytes:
    """Return the canonical provider-only application target."""

    try:
        if not isinstance(binding, Phase6ProviderReconciliationHotfixBinding):
            raise ValueError
    ...
    except Phase6ProviderReconciliationHotfixError:
        raise
    except Exception:
        raise Phase6ProviderReconciliationHotfixError(_GENERIC_ERROR) from None
...
def write_phase6_provider_reconciliation_hotfix(
    binding: Phase6ProviderReconciliationHotfixBinding,
    *,
    ...
    try:
        rendered = render_phase6_provider_reconciliation_hotfix(
...
def verify_phase6_provider_reconciliation_hotfix(
    binding: Phase6ProviderReconciliationHotfixBinding,
    *,
    output_path: Path = DEFAULT_OUTPUT_PATH,
) -> Path:
    ...
    try:
        expected = render_phase6_provider_reconciliation_hotfix(
            _read_repository_file(DEFAULT_PREDECESSOR_PATH),
            binding,
        )
...
def main() -> int:
    ...
    try:
        ...
        output = (
            write_phase6_provider_reconciliation_hotfix(
                binding,
                output_path=arguments.output,
            )
            if arguments.write
            else verify_phase6_provider_reconciliation_hotfix(
                binding,
                output_path=arguments.output,
            )
        )
        rendered = _read_repository_file(output)
    except Phase6ProviderReconciliationHotfixError as error:
        print(str(error))
        return 2
    print(
        json.dumps(
