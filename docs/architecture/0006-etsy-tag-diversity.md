...
Mr Lister asks the model for exactly 13 distinct, buyer-relevant phrases covering the subject,
concrete visual elements, style or aesthetic, visible wording, theme, audience, and buyer intent.
It explicitly discourages repeating subject, product, gift, audience, style, or theme keywords
unless the overlap is essential to a genuinely different search phrase.
...
`tag_keyword_reuse_count`. The model receives a bounded repair opportunity, but its rationale or
claim of compliance is never treated as evidence. If repetition remains, the draft is preserved
in `needs_revision`; Mr Lister performs no production write and permits no approval until a human
supplies a passing tag set. Stop words are ignored, and the system must not substitute irrelevant
filler merely to make the metric pass.
